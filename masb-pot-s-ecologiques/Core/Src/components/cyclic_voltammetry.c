...
void Mesurant_CV(struct CV_Configuration_S cvConfiguration) {
    ...
    // La variable cambio no puede ser un uint (UNSIGNED integer), sino el signo lo perdemos
    // Podriamos hacerlo int (con signo), pero lo hacemos directemente float ya que las operaciones
    // en la que estara involucrado son con decimales y nos ahorramos tener que ir conviertiento
    // el tipo de variable de entero a decimal todo el rato
    float cambio = 1; // creamos una variable que nos permitirá realizar los incrementos o decrecimientos
    ...
    while (cycles) { // mientras el contador sea más pequeño que el número de ciclos
        ...
        if (mesura_punt == TRUE) { // si ha pasado el sampling period
            ...
            } else if (equalsDecimals(vObjetivo, cvConfiguration.eVertex2, TOL)) { // si es igual al vértice 2
            ...
            MCP4725_SetOutputVoltage(hdac, calculateDacOutputVoltage(V_CELL_pre));

            if (equalsDecimals(V_CELL_pre, vObjetivo, TOL)) { // si V_cell es igual a V_objetivo
                ...
                } else if (equalsDecimals(vObjetivo, cvConfiguration.eVertex2, TOL)) { // si es falso
